...
/* this is to force MPI_File_* symbols to be linked to the executable */
#if 0
void dumb_trick(void)
{
  MPI_File fh;
  MPI_File_open(MPI_COMM_WORLD, "dumb_trick", 0, MPI_INFO_NULL, &fh);
}
#endif
...
void h5_initialize_file_(hid_t* file_identifier, char filename[81])
{
  ...
  *(local_filename+len) = '\0';
  ...
  //ierr = MPI_Info_set(FILE_INFO_TEMPLATE, "romio_cb_write", "true");
  //ierr = MPI_Info_set(FILE_INFO_TEMPLATE, "romio_no_indep_rw", "true");
  //ierr = MPI_Info_set(FILE_INFO_TEMPLATE, "cb_config_list", "*:1");
...
#ifdef BGL
  /* TODO: what are the good hints for BGL? */
#endif
