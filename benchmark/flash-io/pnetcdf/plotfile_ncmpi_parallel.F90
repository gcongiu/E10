...
!-----------------------------------------------------------------------------
      ...
      if (corners) then
         filename = trim(basenm) // 'ncmpi_plt_crn_'// fnum_string
      else
         filename = trim(basenm) // 'ncmpi_plt_cnt_'// fnum_string
      endif
...
! get the subset of the variable labels, corresponding to what we are storing
      ...
      call ncmpi_write_header_info_sp(MyPE, & 
     &                             num_out, & 
     &                             file_id, & 
     &                             date_string, &
     &                             flash_release(), &
...
! store the refinement level
      call ncmpi_write_lrefine_sp(file_id,  & 
     &                      varid(1), &
     &                      lrefine,  & 
     &                      lnblocks,  & 
     &                      tot_blocks,  & 
...
! store the nodetype
      call ncmpi_write_nodetype_sp(file_id,  & 
     &                       varid(2), &
     &                       nodetype,  & 
     &                       lnblocks,  & 
     &                       tot_blocks,  & 
...
! store the global id
      call ncmpi_write_gid_sp(file_id,  & 
     &                  varid(3), &
     &                  gid,  & 
     &                  lnblocks,  & 
     &                  tot_blocks,  & 
...
! store the coordinates
      ...
      enddo
      call ncmpi_write_coord_sp(file_id,  & 
     &                       varid(4), &
     &                       xyz_single,  & 
     &                       lnblocks,  & 
     &                       tot_blocks,  & 
...
! store the block size
      call ncmpi_write_size_sp(file_id,  & 
     &                      varid(5), &
     &                      xyz_single,  & 
     &                      lnblocks,  & 
     &                      tot_blocks,  & 
...
! store the bounding box
     ...
     &        real(bnd_box(:,:,block_no), kind = single)
      ...
      call ncmpi_write_bnd_box_sp(file_id, & 
     &                         varid(6), &
     &                         bnd_single, & 
     &                         lnblocks,  & 
     &                         tot_blocks,  & 
...
! we now have the data at the corners, in a 4-byte real array
            ...
            call ncmpi_write_unknowns_sp(file_id, & 
     &                                varid(6+ivar), &
     &                                nxb+1,   & 
     &                                nyb+k2d,   & 
     &                                nzb+k3d, & 
