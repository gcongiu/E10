...
void ncmpi_write_header_info_(int* MyPE,
			   ...
			   int varid[])		      /* output: var ids */
{
	...
	rank = 2;
	dimids[1] = dim_NDIM;
	status = ncmpi_def_var (ncid, "coordinates", NC_DOUBLE, rank, dimids, varid+3);

	/* define var for grid block size */
	rank = 2;
	dimids[1] = dim_NDIM;
	status = ncmpi_def_var (ncid, "blocksize", NC_DOUBLE, rank, dimids, varid+4);

	/* define var for grid bounding box */
	rank = 3;
	dimids[1] = dim_NDIM;
	dimids[2] = dim_2;
	status = ncmpi_def_var (ncid, "bndbox", NC_DOUBLE, rank, dimids, varid+5);
	...
	for (i=0; i<*nvar_out; i++) {
	  ...
	  *(record_label+4) = '\0';
	  status = ncmpi_def_var (ncid, record_label, NC_DOUBLE, rank, dimids, varid+6+i);
	}
...
void ncmpi_write_lrefine_(int* file_identifier,
		       int* varid,
		       int lrefine[],
		       int* local_blocks,
		       int* total_blocks,
...
void ncmpi_write_nodetype_(int* file_identifier,
			int* varid,
			int nodetype[],
			int* local_blocks,
			int* total_blocks,
...
void ncmpi_write_gid_(int* file_identifier,
		   int* varid,
		   int global_id[][NGID],
		   int* local_blocks,
		   int* total_blocks,
		   int* global_offset)
{
  ...
  ncmpi_put_vara_int_all(ncid, *varid, start_2d, count_2d, (const int *)global_id);
}


/* xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx */

void ncmpi_write_coord_(int* file_identifier,
		     int* varid,
		     double * coordinates,
		     int* local_blocks,
		     int* total_blocks,
		     int* global_offset)
{
  ...
  ncmpi_put_vara_double_all(ncid, *varid, start_2d, count_2d, coordinates);
}


/* xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx */

void ncmpi_write_size_(int* file_identifier,
		    int* varid,
		    double * size,
		    int* local_blocks,
		    int* total_blocks,
		    int* global_offset)
{
  ...
  count_2d[0] = (MPI_Offset) (*local_blocks);
  count_2d[1] = NDIM;
  ncmpi_put_vara_double_all(ncid, *varid, start_2d, count_2d, size);
}


/* xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx */

void ncmpi_write_bnd_box_(int* file_identifier,
		       int* varid,
                       double * bnd_box,
                       int* local_blocks,
                       int* total_blocks,
                       int* global_offset)
{
  ...
  ncmpi_put_vara_double_all(ncid, *varid, start_3d, count_3d, bnd_box);
}

/* xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx */


void ncmpi_write_unknowns_(int* file_identifier,
			int* varid,
			...
			int* global_offset)
{
  ...
  ncmpi_put_vara_double_all(ncid, *varid, start_4d, count_4d, unknowns);
}
