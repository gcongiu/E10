...
char *ADIOI_Dirname( const char *str )
{
    char *slash, *dest;

    dest = ADIOI_Strdup( str );
...
char *ADIOI_Abspath( const char *str )
{
    char *ptr = NULL, *abs = ( char* )ADIOI_Malloc( PATH_MAX );
    ptr = realpath( str, abs );
    if( ptr == NULL )
    {
        /* the path was already absolute */
        ADIOI_Strncpy( abs, str, strlen( str ) );
        ptr = abs;
    }
    return ptr;
}
...
{
    char *slash, *dest, join[] = "/\0";
    size_t destsize;
    
    /* NOTE: just use realpath ? */
    slash = strrchr( filename, '/' );
    if( slash != NULL )
        slash++; // skip '/'
    else
        slash = ( char * )filename; // current dir relative path
    destsize = strlen( cachepath ) + strlen( slash ) + 1;
    dest = ( char * )ADIOI_Malloc( destsize );
    strcpy( dest, cachepath );
    strcat( dest, join );
    return strcat( dest, slash );
}
